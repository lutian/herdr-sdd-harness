...
Respeite os exits: `2` = blocked (mostre a tela, espere o humano);
`3` = nenhum executor (pare, alerte, aguarde); `4` = pergunte qual
...
4. Sem APPROVED após N ciclos → `blocked` em `feature_list.json`,
   documente em `progress/current.md` e espere o humano.
5. Diga `Fazendo merge do worktree "…" na branch "{parent}"…` e rode
   `merge-worktree --feature <name>`.
6. Se merge falhar por worktree sujo, peça commit no worktree e retente.
7. Pergunte: `Quer que inicie o fluxo com a <próxima>?`
