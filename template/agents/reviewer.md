...
- ❌ Nunca edite o código do implementador. Seu trabalho é dizer o que
  falha, não consertar. Se o prompt trouxer `HARNESS_ROOT` / `WORKTREE`
  / `capabilities`, leia o código em `$WORKTREE` e escreva o review em
  `$HARNESS_ROOT/sddharness/progress/`. `writeCode` é sempre falso.
